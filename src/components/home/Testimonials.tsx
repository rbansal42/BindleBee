    ...
    return (
        <Section background="sand">
            <div ref={sectionRef}>
                ...
                >
                    <span className="text-sm text-taupe uppercase tracking-[0.2em]">Testimonials</span>
                    <h2 className="font-serif text-3xl md:text-4xl text-walnut mt-3">
                ...
                <div className="grid md:grid-cols-3 gap-8">
                    {testimonials.map((testimonial, index) => (
                        <div
                            key={testimonial.id}
                            className={`bg-cream p-8 rounded-xl transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
                                }`}
                            style={{ transitionDelay: `${(index + 1) * 150}ms` }}
                        >
                            {/* Quote Icon */}
                            <svg
                                className="w-10 h-10 text-terracotta/30 mb-4"
                                fill="currentColor"
                                viewBox="0 0 24 24"
                            >
